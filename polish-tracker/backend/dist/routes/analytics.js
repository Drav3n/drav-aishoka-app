...
router.get('/', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const basicStatsResult = await (0, database_1.query)(`
    SELECT 
    ...
    WHERE user_id = $1
  `, [userId]);
    const brandDistributionResult = await (0, database_1.query)(`
    SELECT 
    ...
    LIMIT 10
  `, [userId]);
    const finishDistributionResult = await (0, database_1.query)(`
    SELECT 
      finish_type,
      COUNT(*) as count
    FROM polishes 
    ...
    ORDER BY count DESC
  `, [userId]);
    const colorDistributionResult = await (0, database_1.query)(`
    SELECT 
      CASE 
    ...
    ORDER BY count DESC
  `, [userId]);
    const mostUsedResult = await (0, database_1.query)(`
    SELECT 
      p.*,
    ...
    LIMIT 5
  `, [userId]);
    const leastUsedResult = await (0, database_1.query)(`
    SELECT 
      p.*,
      b.name as brand_name,
    ...
    LIMIT 5
  `, [userId]);
    const neverUsedResult = await (0, database_1.query)(`
    SELECT 
      p.*,
      b.name as brand_name
    FROM polishes p
    ...
    LIMIT 10
  `, [userId]);
    const usageByMonthResult = await (0, database_1.query)(`
    SELECT 
    ...
    ORDER BY month DESC
  `, [userId]);
    const collectionGrowthResult = await (0, database_1.query)(`
    SELECT 
...
router.get('/summary', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    WHERE user_id = $1
  `, [userId]);
    const recentUsageResult = await (0, database_1.query)(`
